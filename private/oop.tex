...
\begin{enumerate}
    ...
    \item It wil lreturn an error when converted to a list.
...
In modern OOP languages, functions are treated as \emph{first-class} citizens, meaning they can be assigned to variables, passed as arguments, and returned from other functions.
...
In \code{greet} function, the function is assigned to a variable \code{greeting}, which can then be called like a regular function. 
...
result_add = apply_operation(add, 10, 5)       # Output: 15
result_subtract = apply_operation(subtract, 10, 5)  # Output: 5

print(result_add, result_subtract)
\end{lstlisting}

For the above example, the \code{apply\_operation} function takes another function as an argument and applies it to the given arguments. This allows for dynamic behavior based on the function passed to \code{apply\_operation}.



\paragraph{History} Lisp, developed by John McCarthy in the late 1950s, was one of the first programming languages to treat functions as first-class citizens. Lisp's approach to functions was heavily influenced by \emph{lambda calculus}, developed by Alonzo Church in the 1930s, which formalized the concept of functions as mathematical expressions. Lisp's support for first-class functions enabled powerful programming techniques, such as higher-order functions and functional programming paradigms.
...
Because functions are first-class citizens, they can be passed as arguments to other functions, returned from functions, and assigned to variables. Functions that operate on other functions are called \emph{higher-order functions}. More specifically, a higher-order function is a function that takes one or more functions as arguments or returns a function as its result.
...
The example above demonstrate higher-order functions, where functions are passed as arguments to other functions. In this case, the \code{apply\_to\_list} function takes a function and a list of numbers as arguments and applies the function to each number in the list, returning a new list with the results. 
...
\begin{itemize}
    \item \code{map(func, iterable)}: Applies a function to each item in an iterable (e.g., list, tuple) and returns a new iterable with the results.\\
    Example: \code{list(map(square, [1,2,3,4,5]))} returns \code{[1, 4, 9, 16, 25]}.
    \item \code{filter(func, iterable)}: Filters elements in an iterable based on a predicate function (i.e., a function that returns a boolean value).\\
    ...
    \item \code{reduce(func, iterable)}: Applies a function of two arguments cumulatively to the items of an iterable, reducing it to a single value.\\
    Example: \code{reduce(lambda x, y: x + y, [1,2,3,4,5])} returns \code{15}. 
\end{itemize}

\paragraph{Fun fact} While many functional programmers saw \code{reduce} as an essential tool, the Python community largely agreed that list comprehensions and generator expressions made the code more readable than \code{reduce} in many cases. Thus, in Python 3, \code{reduce} was moved to the \code{functools} module to emphasize its specialized use case.
...
\paragraph{Fun fact} Google’s MapReduce framework, introduced in 2004, was inspired by \code{map} and \code{reduce} (``map'' distributes work across multiple nodes and the ``reduce'' aggregates the results).  It allows the processing of massive datasets across distributed systems and is critical for Google in indexing the web efficiently. The framework revolutionizes large-scale data processing and influences technologies like Apache Hadoop and Apache Spark.

\section{Anonymous and Lambda Functions}\label{sec:lambda}
A popular use case for first-class functions is the creation of \emph{anonymous} or \emph{lambda} functions, which are small, unnamed functions defined on the fly. Lambda functions are useful for short, simple operations that do not require a full function definition. These functions are often used in higher-order functions like \code{map}, \code{filter}, and \code{reduce} as discussed in the previous section.
\begin{lstlisting}
# Lambda function to square a number
square = lambda x: x ** 2
print(square(5))  # Output: 25

# Lambda function used in a higher-order function
numbers = [1, 2, 3, 4, 5]
