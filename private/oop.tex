...
\begin{enumerate}
    ...
    \code{def ga(pop\_size:int, list\_size:int, xover\_rate:float, mut\_rate:float, min\_val:int, max\_val:int, target:int): -> (list[int], float)}
    \item Your GA should print out the best individual, its sum, and its fitness score at each generation (iteration).
    \item Your GA has the following various parameters (inputs to the \code{ga}). You should play around with them to find values that work best.  You can start with these values: \code{pop\_size=100, list\_size=10, xover\_rate=0.8, mut\_rate=0.1, min\_val=-100, max\_val=100, target=1000}.
    \item Test your GA with various parameters and sum targets.  You should also try different selection methods, crossover methods, and mutation methods.
