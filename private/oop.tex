...
For function \code{f} above we simply test it with various numbers as inputs and check that the output is as expected. We do not need to know how \code{square} is implemented. Note that because this is blackbox testing, we do not know about the special case of \code{123} and thus do not test for it. This is a limitation of blackbox testing.

\subsection{Special Cases Testing}

This testing runs the program on special or edge cases to find bugs that are not caught by regular inputs. For example, a program that computes the square of a number (like \code{f} above) should be tested with negative numbers, zero, and positive numbers. Similarly, for a program that takes a list of numbers as input, special cases could include an empty list, with one element, with all 0's, with all negative numbers, etc.


\subsection{Fuzz Testing}
This testing uses random or invalid inputs to test the program. For example, a program expects a number is tested with a string or a dict.  It has the similar purpose as special cases testing, but instead of using specific inputs, it generates many random / invalid inputs. Fuzz testing is often used to find security vulnerabilities. More advanced fuzzing techniques generate new inputs from existing ones, e.g., by flipping bits or changing values slightly.
...
# unit testing for the square function
def test_square():
...
Genetic Algorithm (GA) is a test generation technique that uses biological evolution (Darwin's theory of evolution) to generate test inputs.

GA starts with an initial set or \emph{population} of random inputs (\emph{individuals}) and iteratively evolves them to find the best one that achieves some testing objective (e.g., maximizing code coverage, causing a crash). GA uses a \emph{fitness function} to evaluate the quality of the individuals and \emph{selects} the best ones to survive and reproduce. Next, GA applies genetic operators such as \emph{mutation} and  \emph{crossover} to create individuals representing the new population. This process continues until a \emph{stopping criterion} is met.
...
You will implement a GA that evolves a population of lists of integers to find a list whose sum is a given target sum. You can use the GA template above for this task. Specifically, you will implement the following GA components:
...
\begin{enumerate}
    ...
    \code{gen\_pop(pop\_size: int, list\_size: int, min\_val: int, max\_val: int) -> list[list[int]]}
    ...
    \code{get\_fitness(individual: list[int], target\_sum: int)}
    ...
    \code{def mutate(individual: list[int], rrate: float, min\_val: int, max\_val: int) -> list[int]}
    ...
    \code{def stopping\_criterion(fitness: list[int], target\_sum: int) -> bool}
    \item Write the main genetic algorithm that uses all the above functions and returns the best individual\\
    \code{def ga(pop\_size: int, list\_size: int, xover\_rate: float, mut\_rate: float, min\_val: int, max\_val: int, target\_sum: int): -> list[int]}
    \item Your GA should print out the best individual, its sum, and its fitness score at each generation (iteration).
    \item Your GA has the following various parameters (inputs to the \code{ga}). You should play around with them to find values that work best.  You can start with these values: \code{pop\_size=100, list\_size=10, xover\_rate=0.8, mut\_rate=0.1, min\_val=-100, max\_val=100, target\_sum=1000}.
    \item Test your GA with various parameters and target sums.  You should also try different selection methods, crossover methods, and mutation methods.
    \item Write a report that explains your GA. More specifically for each of the above task, explain that you did (e.g., how do you generate the population, how do you compute the fitness, etc).
    \item In your report, explain the parameters you used and how they affect the performance of the GA.  For example, how does the population size affect the performance? crossover and mutation rates? etc.
    \item Submit your report and code, as well as screenshots of your GA running with various parameters and target sums.
\end{enumerate}
