...
\item Part 3: Create some code that applies \code{reduce} and discuss about it in class

\item Part 4: Modify the code to use higher-order functions
\begin{enumerate}
\item Create some code that applies \code{map}, \code{filter}, and \code{reduce} to a list of string
\item Explain how \code{reduce} works on a list \code{[1,2,3,...,n]}
\end{enumerate}

\item Part 4: Create a \emph{closure} that stores a base number and uses it to multiply with other numbers.
\begin{enumerate}
...
\item Part 5: Combine all three concepts to create a flexible higher-order function that applies a lambda function within a closure.
