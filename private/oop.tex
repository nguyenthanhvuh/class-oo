...
\begin{enumerate}
    ...
    \item It is faster than using a list.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item It returns an error when converted to a list.
    \item It gets exhausted and return an empty list.
...
\item What is the purpose of the following generator?
\begin{lstlisting}
...
\begin{enumerate}
    ...
    \item It generates numbers starting from 0, but stops after a certain point.
...
\item Part 3: Modify the code to use higher-order functions
\begin{enumerate}
\item Create some code that applies \code{map}, \code{filter}, and \code{reduce} to a list of string
\item \code{reduce} also takes a third parameter called an \emph{accumulator}.  Explain how \code{reduce} works on a list \code{reduce(f,[1,2,3,...,n],acc)}.
\end{enumerate}

\item Part 4: Write a function \code{make\_max\_tracker()} that returns a closure that tracks and returns the highest number seen so far.
\begin{lstlisting}
    def max_tracker():
        ...
        def tracker(v):
...
\item Part 5: Create a \emph{closure} that stores a base number and uses it to multiply with other numbers.
\begin{enumerate}
...
\item Part 6: Combine all three concepts to create a flexible higher-order function that applies a lambda function within a closure.
