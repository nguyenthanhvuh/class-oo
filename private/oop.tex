...
\subsection{E1}
\begin{enumerate}
    \item Explain the difference between a higher-order function and a closure. Provide an example of each.
    \item When would you use a lambda function over a regular function? Provide an example.
    \item Write a function \code{make\_min\_tracker()} that returns a closure that tracks and returns the lowest number seen so far.
    \begin{lstlisting}
    ...
    print(min_tracker(5))  # Output: 5
    print(min_tracker(3))  # Output: 3
    print(min_tracker(8))  # Output: 3
    print(min_tracker(-1)) # Output: -1
    print(min_tracker(0))  # Output: -1
    \end{lstlisting}
\end{enumerate}
...
You will often see the terms \emph{validation}, \emph{verification}, and \emph{testing} used in software engineering. \textbf{Validation} is a process typically achieved by verification and validation to ensure the program behaves as expected. \textbf{Verification} ensures that the program works on \emph{all possible inputs}, which is expensive or impossible for large programs.

\textbf{Testing}, on the other hand, checks that the program behaves as expected over \emph{some inputs}, which is usually cheaper. Software developers are often more familiar with testing, e.g., by running the program with various inputs. We focus on testing in this chapter.  
...
\begin{lstlisting}
    ...
    def ga():
        ...
        while not stopping_criterion(fitness):
        ...
        best = select_best(pop, fitness)
        return best
\end{lstlisting}

\paragraph{Example: Evolving a List of Numbers to Match a Target List}

	•	Objective: Generate a list that matches a target list.
	•	Example: Given a target list of [5, 3, 8, 1], the GA evolves random lists toward matching this target exactly. The fitness function measures the similarity (e.g., using mean squared error or absolute difference between elements) between the evolving list and the target list.
	•	Key Learning: This example illustrates how GAs can optimize toward specific known solutions, making it clear how mutation and crossover help move toward the target over generations.
\begin{lstlisting}
    ...
    def mutate(individual, mutation_rate):
        return [x + random.uniform(-mutation_rate, mutation_rate) for x in individual]

    def ga(target, pop_size, mutation_rate):
        # Initialize the population
        pop = [gen_individual() for _ in range(pop_size)]

        # Evaluate the fitness of each individual
        fitness = [fitness(individual, target) for individual in pop]
        ...
        while max(fitness) > 0:
            ...
            # Evaluate the fitness of the new population
            fitness = [fitness(individual, target) for individual in offspring]
