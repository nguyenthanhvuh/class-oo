...
Genetic Algorithm (GA) is a test input generation technique that uses biological evolution (Darwin's theory of evolution) to generate test inputs.

GA starts with an initial set or \emph{population} of random inputs (\emph{individuals}) and iteratively evolves them to find the best one that achieves some objective (e.g., maximizing code coverage, causing a crash). GA uses a \emph{fitness function} to evaluate the quality of the individuals and \emph{selects} the best ones to survive and reproduce. Next, GA applies genetic operators to create individuals representing the new population. This process continues until a \emph{stopping criterion} is met.
