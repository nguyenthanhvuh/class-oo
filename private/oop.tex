...
\begin{lstlisting}
    """
    combinatorial testing for division
    """
...
For the example above, the \code{test\_square} and \code{test\_add} functions are run with random integers \code{x, y}. Notice that in Python, for property-based testing we use the \code{hypothesis} library, which generates random inputs and runs the tests with them.
...
SBST searches for inputs to maximize (or minimize) some objective. Examples of such objectives include maximing code coverage, causing a crash, or satisfying a specific property.
