...
\paragraph{Example: using GA to generate list of large numbers}
\begin{lstlisting}
    def gen_pop():
        return [random.randint(1000, 10000) for _ in range(100)]

    def eval_fitness(pop):
        return [sum(x) for x in pop]

    def stopping_criterion(fitness):
        return max(fitness) > 100000

    def select_best(pop, fitness):
        return pop[fitness.index(max(fitness))]

    def select(pop, fitness):
        return [pop[i] for i in range(len(pop)) if fitness[i] > 50000]

    def crossover(parents):
        return [parents[0] + parents[1] for _ in range(100)]

    def mutate(offspring):
        return [x + random.randint(-100, 100) for x in offspring]
