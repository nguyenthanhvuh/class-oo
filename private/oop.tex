...
\item Implementation satisfying given specification: Define a Python class called `Loan`. In that class, write a function that satisfies the following specification:
...
class Loan:
    ...
    \begin{itemize}
        \item Note that the precondition is quite strong, which makes implementing the method easy. The key step in your calculation is to change the principal on each iteration with the following formula (which amounts to monthly compounding):
        \begin{lstlisting}
...
Use the Poly ADT in \autoref{ex:poly} to answer the following question. Use the \code{Stack} ADT in \autoref{ex:stack} as an example. 

\begin{enumerate}
\item Write an Overview that describes what \code{Poly} does. You must provide some examples to demonstrate (e.g., Poly(2,3) means what?).
...
\emph{Iterators} provide general-purpose iteration mechanism that is both efficient and preserves abstraction. An iterator is a special kind of object that allows us to access elements one at a time, without creating large intermediate structures or destroying the original collection.

Each iteration produces a new item that can then be used within the loop. The key advantage of iterators is the \emph{separation of concerns}: the iterator handles the production of items, while the code containing the loop handles the actions performed on those items. The iterator can be reused in different contexts without modification.

Because items are produced incrementally, iterators avoid the space and time inefficiencies discussed earlier. They do not require constructing large intermediate structures (such as lists), and the iteration can stop early if a certain condition is met (e.g., finding a specific item).
...
In this example, \code{IntSet} defines an \code{\_\_iter\_\_} method, which returns an iterator over its elements. This allows us to use Python's built-in \code{sum()} function to iterate over the elements without destroying the set.

\section{Iterators and Generators}

%A modern OOP language such as Python provides support for iteration abstraction through its built-in iterator and generator mechanisms. In Python, an iterator is an object thatadheres to a specific interface, providing methods like __iter__() and __next__(). Iterators track their state internally, and they have a next() method (in Python, it’s called __next__()), which advances the iteration and returns the next element, and a hasNext() equivalent can be checked using exception handling or by attempting to iterate further.

In Python, a generator is a type of iterator created using special syntax (e.g., \code{yield} keyword), which allows elements to be produced one at a time, maintaining the state of the iteration.
