...
\emph{Procedural abstraction} is a fundamental programming concept. It allows developers to create functions (methods) that hide program implementation details. For example, the user can just call \code{sort} without knowing (or caring) about the sorting algorithm used. 
...
The terms \emph{validation}, \emph{verification}, and \emph{testing} are used in software engineering. \textbf{Validation} is a process typically achieved by verification and validation to ensure the program behaves as expected. \textbf{Verification} ensures that the program works on \emph{all possible inputs}. Verification provides better guarantee but is expensive or impossible for large programs.

\textbf{Testing}, on the other hand, checks that the program behaves as expected over \emph{some inputs}. Testing only shows the program works on the test inputs,  but it is usually cheaper to do (comparing to verification). Software developers are often more familiar with testing, e.g., by running the program with various inputs. We focus on testing in this chapter.  
...
Black-Box testing approach checks the program using its specifications (e.g., type of inputs, expected outputs) without any knowledge of its internal implementation. This approach ignores the implementation details and does not even require the program code (hence the name black-box).  The approach is efficient and easy to use, but can miss certain bugs.
...
\subsection{Special Cases Testing}
...
This testing generates random and invalid inputs to test the program. For example, a program expects a number is tested with a string or a dict.  It has the similar purpose as special cases testing, but instead of using specific inputs, it generates many random / invalid inputs. Fuzz testing is often used to find security vulnerabilities. More advanced fuzzing techniques generate new inputs from existing ones, e.g., by flipping bits or changing values slightly.
...
This technique combines different inputs to generate tests.  The combination is typically done using \emph{Cartesian} products, i.e., all possible combinations of inputs are tested. In addition to generating more tests, combinatorial testing is useful for finding issues that occur when combining different inputs. For example, a program that takes two numbers as input could be tested with all combinations of positive, negative, and zero numbers. 
...
For the example above, the \code{test\_div} function is run with all combinations of numbers in \code{xs} and \code{ys}, i.e., 25 tests. The \code{product} function generates all combinations of the lists \code{xs, ys} (cartesian product). The \code{@parameterized.expand} runs the test with each (combined) inputs. Note that while this is illustrated using Python, the concept of combinatorial testing is used in other languages and testing frameworks.

\subsection{Property-Based Testing}
This checks that certain properties hold over randomly generated inputs. Examples of properties include the square of a negative number is positive and addition and multiplication being commutative (e.g., $x + y \equiv y + x$). Property-based testing is a convenient way to generate and test the program with many inputs.
...
The \code{test\_square} and \code{test\_add} functions are run with random integers \code{x, y}. Notice that in Python, for property-based testing we use the \code{hypothesis} library, which generates random inputs and runs the tests with them.


\section{In-class Exercise: GCD}
You will practice combinatorial and property testing using the \emph{Greatest Common Divisor} (GCD) function. The GCD of two numbers is the largest number that divides both of them. For example, the GCD of 8 and 12 is 4.  You are given 2 implementations of the GCD function (using Euclidean algorithm), one of them is correct and the other has a bug.  You will write combinatorial and property tests to find the bug.
...
\paragraph{Part 1: Combinatorial Testing}

\paragraph{Part 2: Property Testing}



\subsection{Search-Based Software Testing (SBST)}
SBST searches for inputs to optimize some objective. Examples of such objectives include maximizing code coverage, causing a crash, or satisfying a specific property.
...
GA starts with an initial set or \emph{population} of random inputs (\emph{individuals}) and iteratively evolves them to find the best one that achieves some objective. GA uses a \emph{fitness function} to evaluate the quality of the individuals and \emph{selects} the best ones to survive and reproduce . GA applies genetic operators to create individuals representing the new population in the next \emph{generation}. This process continues until a \emph{stopping criterion} is met.

The main genetic operators in a GA are
\begin{enumerate}
    \item \emph{Crossover}: combines two individuals or \emph{parents} to create new ones. Common xover methods include single-point, two-point, and uniform crossover. xover rate is the probability of applying xover, and typically is high (e.g., 0.8 or 80\% chance).
    \item \emph{Mutation}: randomly changes some elements of an individual. Common mutation methods include randm (create a random value), swap (swap two elements), and flip (flip a bit 0 to 1 and vice versa). Mutation rate is the probability of doing mutation, and typically is low (e.g., lower than 0.1 or 10\% chance).
\end{enumerate}

The following is a template for GA:
\begin{lstlisting}
...
\paragraph{Task 1: GA implementation}
 ...
 You can use the GA template in \autoref{sec:ga} for this task. You can also use the following \href{https://nguyenthanhvuh.github.io/class-oo/ga-example.py}{GA code for counting 0's} as example. Note that you will likely need to modify this code to fit your needs as the problem and objective are very different.
 
 
 Specifically, you will implement the following GA components. Note that the \emph{given signatures} below of the functions are just suggestions. You can modify them as needed.


\begin{enumerate}
    \item Generate a random population of lists of integers. The length of the popular and individual lists are given as input.  The integers in the list should be between a specified range (e.g., -100,100)\\
    \code{gen\_pop(pop\_size:int, indv\_size:int, min\_val:int, max\_val:int) -> list[list[int]]}
    \item Write a \emph{fitness function} that computes the fitness score based on how close the sum of the list is to the target sum. Closer is better (e.g., if the target is 99, then a list whose sum is 99 should have the ``perfect score'' while a list whose sum is 90 has a better score than a list whose sum is 50). Note that you must also taken account of negative numbers and sums.  \\
    ...
    \item Write a \emph{crossover} function that takes two parents and creates two offsprings using \emph{single-point} crossover (i.e., pick a random point and swap the tails of the parents).\\
    ...
    \item Write the main genetic algorithm that uses all the above functions and returns the best individual and its fitness\\
    ...
    \item Your GA has the following various parameters (inputs to the \code{ga}). You should play around with them to find values that work best.  You can start with these values: \code{pop\_size=100, indv\_size=10, xover\_rate=0.8, mut\_rate=0.1, min\_val=-100, max\_val=100, target=1000}.
    \item Test your GA with various parameters and sum targets.  You can also try different selection methods, crossover methods, and mutation methods.
...
\paragraph{Task 2: Write a short report}
\begin{enumerate}
