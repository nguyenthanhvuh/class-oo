...
Testing is a critical component of software development to ensure that the program behaves as expected (e.g., satisfying method specifications and rep-invariants in OOP). Testing also reveal bugs and errors in the implementation and thus save time and resources.  

\section{Validation, Verification, and Testing}
...
Black-Box testing approach checks the program strictly based on its specifications without any knowledge of its internal implementation. This approach ignores the implementation details and does not even require the program code (hence the name black-box).  The approach is efficient and easy to use, but can miss certain bugs.
...
This technique runs the program on special or edge cases to find bugs that are not caught by regular inputs. For example, a program that computes the square of a number (like \code{f} above) should be tested with negative numbers, zero, and positive numbers. Similarly, for a program that takes a list of numbers as input, special cases could include an empty list, a list with one element, a list with all negative numbers, etc.


\subsection{Fuzz Testing}
This technique uses random or invalid inputs to test the program. For example, a program expects a number is tested with a string or a dict.  It has the similar purpose as special cases testing, but instead of using specific inputs, it generate many random / invalid inputs. Fuzz testing is often used to find security vulnerabilities. More advanced fuzzing techniques generate new inputs from existing ones, e.g., by flipping bits or changing values slightly.

\begin{lstlisting}
def test_square_fuzz():
