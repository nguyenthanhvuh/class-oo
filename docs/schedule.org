...
*** Goals: Type Abstraction
    ...
    1. Suppose that offers are only accepted if they are lower than previous offers.
       ...
       #+export_comment
       Pre:  OK, doesn't have pre so weaker
       Post: NotOK, not stronger,  before it works with all item, but now it does not 
       #+export_comment
    ...
    1. Suppose that the =buy()= method always chooses the lowest price on an item.
       ...
       #+export_comment
       Pre:  OK, same
       Post: OK, stronger, supertype removes arbitrary one, subtype removes specific one
       #+export_comment
