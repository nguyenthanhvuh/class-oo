#+TITLE: Course Schedule and Assignments
#+OPTIONS: ^:nil toc:1
...
* Schedule
  ...
  |     <r> |                | <l>                   |       <c>       |              <c>               |   |   |   |
  |---------+----------------+-----------------------+-----------------+--------------------------------+---+---+---|
  |       1 | Class Overview | Liskov 1--3           |                 | IC 1 [[#ic1A][A]] [[#ic1B][B]] |   |   |   |
  |       2 | Exceptions     | Liskov 4, Bloch 10    | [[#a1][A1 Due]] | IC 2 [[#ic2A][A]] [[#ic2B][B]] |   |   |   |
  # |       3 | Data Abstraction; Mutability             | Liskov 5.1--5.4,                                                 |  [[#a2][A2 Due]]  |          IC 3 [[#ic3A][A]] [[#ic3B][B]]          |   |
  # |       4 | Imperative Program Verification          | [Hoare logic notes]                                              |  [[#a3][A3 Due]]  |                                                  |   |
  # |       5 | Imperative Program verification          | [Hoare logic notes]                                              |                   |              IC        [[#ic4][4]]               |   |
  ...
  # |       9 | Mid-semester Recap; [Research slides]    | /No New Material/                                                |                   |                  IC [[#ic8][8]]                  |   |
...
** In class 1A
   ...
   1. Decide on a mechanism for joint communication. Google docs? IDE with screen share? Zoom? Something else?

   Now, address a technical topic. With your group, discuss what /preconditions/ and /postconditions/ are.
...
** In class 1B
   ...
   :END:
    ...
    Consider the Java [[https://docs.oracle.com/javase/8/docs/api/java/util/Iterator.html][Iterator interface]]. 
    1. For each method, identify all preconditions and postconditions. Pay special attention to possible exceptions.
    1. For each precondition, identify a specific input that violates the precondition.
    1. What "happens" if the precondition is violated? You should answer this question both theoretically and practically. (Hint: Really, it's the same answer...)
...
*** Goals
    ...
    1. Your *group* should communicate the composition of your group to me (and the grader) on Piazza. If you group is sticking with the random assignment, just confirm that. If you have a new group, tell us the composition, and we'll edit the post to reflect the change.
    1. Consider the following specification:
     ...
     Implement this method in a class called =FindPrimeFactor=. Although you should (obviously) test your implementation yourself, you do not need to include any test code. The grader will test your code. Your solution should be minimal in the sense that it doesn't include any unnecessary code. In particular, error checking in =findPrimeFactor()= for parameters that do not satisfy the precondtions is not appropriate for this assignment.
...
** Guide 1
   ...
   Quiz 1 will be a gentle introduction to 419 quizzes. The quiz will focus on the contract model. You should be able to answer basic questions about preconditions and postconditions.

   I also want students to read the syllabus carefully. Experience has shown that some students need a bit of prodding to do this. Hence, questions about the syllabus are also fair game.
...
** Reflection 3
   ...
   1. Please explain what you think it means to to correctly override the toString() method. Base your answer on your understanding *before* enrolling in this class.
   1. How do you decide whether you have implemented a Java method correctly? Again, base your answer on your understanding *before* enrolling in this class.
