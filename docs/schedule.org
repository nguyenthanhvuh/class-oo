...
*** Goals: Understanding Program Verification through Hoare Logic
    ...
    Redo the in-class exercise and submit it. More specifically, you will do the below two tasks:
      1. Prove the program using the following the loop invariant:  =unsufficient=.  Clearly reason why this is a loop invariant, compute =wp=,  compute =vc=, and reason about the =vc=, conclude whether the program is proved or not.
      1. Prove the program using the following the loop invariant:  =unsufficient=.  Clearly reason why this is a loop invariant, compute =wp=,  compute =vc=, and reason about the =vc=, conclude whether the program is proved or not.
