...
-   **Notes**: Except for the first class, students are expected to
    ...
    may occasionally take advantage of this expectation. Homework
    assignments are **due** on the date listed.

    Meeting Topic            Reading                Assignment         In-Class Exercise             
  --------- ---------------- -------------------- --------------- ---------------------------- -- -- --
          1 Class Overview   Liskov 1--3                           IC 1 [A](#ic1A) [B](#ic1B)        
          2 Exceptions       Liskov 4, Bloch 10    [A1 Due](#a1)   IC 2 [A](#ic2A) [B](#ic2B)        
...
2.  For each precondition, identify a specific input that violates the
    precondition.
3.  What \"happens\" if the precondition is violated? You should answer
    this question both theoretically and practically. (Hint: Really,
...
2.  Basic familiarity with contracts and coding
...
3.  Consider the following specification:
    ...
    Implement this method in a class called
    `FindPrimeFactor`{.verbatim}. Although you should (obviously) test
    your implementation yourself, you do not need to include any test
    code. The grader will test your code. Your solution should be
    minimal in the sense that it doesn\'t include any unnecessary code.
    In particular, error checking in `findPrimeFactor()`{.verbatim} for
    parameters that do not satisfy the precondtions is not appropriate
    for this assignment.

### Grading Criteria

-   Your individual Piazza post adhers to my instructions. (That is, no
    sideways pictures, no oversize pictures, etc.)
-   You are in a group.
-   Coding: Adherence to instructions. Do what I ask for above, not
    something else. Code runs correctly.
...
Define a Java class called `Loan`{.verbatim}. In that class, write a
...
-   Now modify `months`{.verbatim} so that it handles **all** of its
...
1.  Prove the program using the following the loop invariant: `i <= N`.
    ...
    2.  Compute the weakest precondition `wp`{.verbatim} of the program
        wrt the post conditiong `Q`{.verbatim}
    3.  Compute the verification condition
        `vc (P => wp(..))`{.verbatim}, and
    4.  Analyze the `vc`{.verbatim} to dertermine whether the program is
        proved or not
...
only break the immutability of the `Period`{.verbatim} class by writing
...
Consider the following `Market`{.verbatim} class.
...
1.  Suppose that offers are only accepted if they are lower than
    ...
    Is `Low_Bid_Market`{.verbatim} a valid subtype of
    `Market`{.verbatim}? Appeal to the methods rule to back up your
    answer.

2.  Suppose that the `buy()`{.verbatim} method always chooses the lowest
    price on an item.
    ...
    Is `Low_Offer_Market`{.verbatim} a valid subtype of
    `Market`{.verbatim}? Appeal to the methods rule to back up your
    answer.
...
A `Comparator`{.verbatim} based on absolute values is problematic. Code
...
two Java sets, one a `HashSet`{.verbatim}, and the other a
`TreeSet`{.verbatim}. The `TreeSet`{.verbatim} should order items with
...
Add `putAll()`{.verbatim} and `getAll()`{.verbatim}. Define the method
signatures carefully. Use exception-handling consistent with that for
`get()`{.verbatim} and `put()`{.verbatim}. Use bounded wildcards as
appropriate. Note that `putAll()`{.verbatim} has a special case where
...
### Goals: `Object`{.verbatim} class contracts.

As it happens, Liskov\'s implementation of `clone()`{.verbatim} for the
`IntSet`{.verbatim} class (see figure 5.10, page 97) is wrong.

1.  Use the [version](./files/IntSet.java) of `IntSet`{.verbatim} from
    the in-class exercise. Implement a subtype of `IntSet`{.verbatim} to
    demonstrate the problem. Your solution should include appropiate
    executable code in the form of JUnit tests.
2.  Provide a correct implementation of `clone()`{.verbatim} for
    `IntSet`{.verbatim}. Again, give appropriate JUnit tests.
3.  Correctly override `hashCode()`{.verbatim} and
    `equals()`{.verbatim}. Note that the standard recipe is not
...
1.  Replace `Set` with `List`. There is no problem with `equals()`. Why
    not?
2.  Replace `Set`{.verbatim} with `Collection`{.verbatim}. Now
    `equals()`{.verbatim} does not satisfy its contract.
...
1.  Consider one of the `copyOf()`{.verbatim} methods in the Java
    [Arrays](https://docs.oracle.com/javase/7/docs/api/java/util/Arrays.html)
    utility class. Bloch uses this method in his `Stack`{.verbatim}
...
2.  For most of the semester, we have focused on design considerations
    ...
    `readObject()`{.verbatim} methods defensively*. One is called
    `BogusPeriod`{.verbatim}, and the other is called
    `MutablePeriod`{.verbatim}. Implement either (your choice) of these
...
4.  Find some existing (Java) code that uses the \"int enum pattern\"
    and refactor it to use Java `Enums`{.verbatim} instead. Identify any
...
5.  Where appropriate, code up, as JUnit theories, constraints for
    classes that implement the Java `Comparable`{.verbatim} interface.
