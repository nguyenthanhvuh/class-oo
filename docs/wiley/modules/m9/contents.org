...
** In this module we will talk about generics, i.e., parametric polymorphism. We will learn the benefits and proper usage of generics. Specifically, we will learn when we should use generics and how to convert code that does not use generics to one that does.
...
1. explain and demonstrate the benefits and key concepts of generics
2. Compare and describe specification terminologies and concepts (e.g., raw types vs. generic types, arrays vs. lists, wild cards)
3. Convert from non-generic code to generic code, from code using arrays to one using lists.
