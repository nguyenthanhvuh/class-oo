...
For preconditions, we want as weak a constraint as possible to make the function more versatile, allowing it to handle a larger class of inputs. 
...
%Liskov 5 
This chapter focuses on \emph{abstract data type} (ADT), a key concept in programming that allows developers to separate how data is implemented from how it behaves. Data abstraction forms the foundation of OOP. Through ADT, programmers can create new data types relevant to their application. These ADTs consist of objects and associated operations.
...
In a modern OOP language such as Python or Java, data abstractions are defined using classes. A specification defines the behavior of an ADT without being tied to a specific implementation. The specification explains what the operations on the data type do, allowing users to interact with objects only via methods, rather than accessing the internal representation.

\paragraph{Structure of an ADT}
A typical specification consists of three main parts:
\begin{enumerate}
\item An overview of the type.
\item Specifications for constructors (which create objects).
\item Specifications for methods (which operate on the objects).
\end{enumerate}

% The form of the data abstraction in Python can be described as follows:
...
% Constructors in Python are defined using the \texttt{\_\_init\_\_} method. The constructor initializes a new object, setting up any initial state required for the instance.

% 3. Methods

% Methods are the operations users can perform on the objects. These methods allow users to interact with the object without needing to know its internal representation. The specifications of methods in Python resemble function specifications, where self is used to refer to the instance of the object on which the method is called (analogous to this in Java).
\section{Reasoning}
\appendix
